 ...
 * @returns {WOQLQuery}
 *
 * NOTE: select() with no arguments is a valid use case for subclauses where outer
 * variables are used but no additional variables from the inner query should be
 * in the result. The validation on line 314-316 may be overly restrictive and
 * should potentially be removed to allow select() with zero variables.
 *
 * ISSUE: Current implementation rejects select() with no arguments, but this
 * prevents valid patterns like:
 *   select().and(triple("v:X", "rdf:type", "v:Person"))
 * where v:X is bound in an outer scope and we want to filter without adding
 * new variables to the result set.
 */

WOQLQuery.prototype.select = function (...varNames) {
  if (this.cursor['@type']) this.wrapCursorWithAnd();
  this.cursor['@type'] = 'Select';
  // Empty variable list is valid - it hides all inner variables
  // This is useful for patterns like localize() where we want to
  // execute a query but not expose any of its internal bindings
  if (!varNames || varNames.length === 0) {
    this.cursor.variables = [];
    return this.addSubQuery(false);
  }
 ...
 * - localized: function that wraps queries with select() (empty variable list) and eq() bindings
